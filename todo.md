...
- **Overlay dispatch in `render.ts` becomes an exhaustive `switch`.** Replace the
  `if`/`continue`/`if`/`if` chain (`:30-40`) with a `switch` on `overlay.type` plus
...
- **Drop `compose.ts`'s `Paragraph` alias; alias CanvasKit's.** The alias is used twice
  ...
  it — consumers already name the inner array `spans` (`text-layout.ts:63,99`).
...
- **`ComposedText` → `ComposedTextOverlay`, `composeText` → `composeTextOverlay`.** Completes
  ...
  one named phase rather than a vague verb. Call sites: `render.ts:5,52`,
  `text-layout.ts:10,54,61,87`.
- **`laid` → `layoutAttempt`.** `text-layout.ts:109`, plus its reads in the shrink loop
  (`:110-112`) and at `:115-125`. `tryLayout` keeps its name.
- **`PX_PER_MM` → `RASTER_PX_PER_MM`.** `page.ts:12`. `PT_PER_MM` is a physical fact,
...
- **The raw deck text is `deckSource` everywhere.** It is unvalidated text parsed line by
  ...
  takes a deck and returns a deck. Sites: `imposition.ts:7,9,11` (`selectFromDeck` param and its two reads);
  `imposition.ts:5,31` (the `Imposition` field); `app/printer.tsx:44,77,87`
  (`Settings` field, debounced local, and where the `Imposition` is built);
  `app/sidebar.tsx:26-27` (textarea binding). `Imposition["kind"]` stays `"deck"` — that
  names the mode, not the text.

# step 2 — needs logic changes

- **Drop the hardcoded `"Bogle"` default font family.** `text-layout.ts:15` hardcodes
  `proxy-db-lorcana`'s typeface as the engine's fallback. Use the DB's own default —
  `db.fonts[0]?.fontFamily`, already computed at `environment.ts:160` but filed under
  `svgFonts` where only resvg reads it — in both places that use the literal: the
  `capRatios` lookup (`text-layout.ts:313`) and `fontFamilies` (`:355`), the latter as a
